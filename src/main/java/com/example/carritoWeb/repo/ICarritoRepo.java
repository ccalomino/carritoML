...
//	List<Carrito> findAllCarritos();
	...
	//------------------------------------------------------------------------------------------	
	@Query(value = "select c.id_carrito, c.fecha, u.id_Usu, p.id_prod, u.nombre as \"cliente\", p.nombre as \"producto\", p.precio, pc.cantidad\n,  (select count(*) from productos_en_carrito pc2 where pc2.idc = c.id_carrito) as \"rows\", v.valor_total as \"total\""
	...
	//------------------------------------------------------------------------------------------
	@Query(value = "select c.id_carrito, c.fecha, u.id_Usu, p.id_prod, u.nombre as cliente, p.nombre as producto, p.precio, pc.cantidad, (select count(*) from productos_en_carrito pc2 where pc2.idc = c.id_carrito) as rows, v.valor_total\n"
			+ "	from usuario u, carrito c, productos_en_carrito pc, producto p, venta v\n"
